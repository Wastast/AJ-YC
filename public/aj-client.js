...
const SERVER_ADDRES = 'http://47.99.79.15:3001/socket';

class AJClient {
  // 初始化 对象时调用
  constructor() {
    this.socket_ = io.connect(SERVER_ADDRES);

    // 监听connect 事件
    this.socket_.on('connect', () => {
      this.isConnect_ = true; // 判断是否初始化成功
      let userinfo = {
        token: 'anji_token',
        uin: 1253,
        id: 10000
      }; // 用户信息
      this.socket_.emit('login_main', { userinfo });
    });
    ...
    // 初始化后的信息
    this.socket_.on('message', data => {
      console.log('message', data);
      if (data.event == 'login_success') {
        let options = data.data;
        this.sdkAppId_ = options.sdkAppId_;
        this.userId_ = options.userId_;
        this.userSig_ = options.userSig_;
        this.roomId_ = options.roomId_;
      }
    });

    this.socket_.on('events', function(data) {
      console.log('event', data);
    });

    this.isConnect_ = false;
    this.isJoined_ = false; // 是否加入成功
    this.isPublished_ = false;
    this.localStream_ = null;
    this.remoteStreams_ = [];
    this.onAddRemoteView = null;
  ...
  async onRemoteView(cb) {
    this.onAddRemoteView = cb;
  }
  ...
  handleEvents() {
    ...
    this.client_.on('stream-subscribed', evt => {
      ...
      if (this.onAddRemoteView) {
        this.onAddRemoteView(id);
      }
      // 在指定的 div 容器上播放音视频
